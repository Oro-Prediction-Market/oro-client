...
describe("SeasonService", () => {
  describe("openNewSeason", () => {
    ...
    it("returns existing season without creating a duplicate", async () => {
      const existing = { id: "existing", weekNumber: 1, year: 2025, status: SeasonStatus.ACTIVE };
      const seasonRepo = makeSeasonRepo();
  ...
  describe("closeActiveSeason", () => {
    ...
    it("closes active season and snapshots top users", async () => {
      ...
      const users = [
        { id: "u1", firstName: "Alice", username: null, reputationScore: 0.9, reputationTier: "legend", totalPredictions: 20, correctPredictions: 18 },
        { id: "u2", firstName: "Bob", username: "bob", reputationScore: 0.7, reputationTier: "hot_hand", totalPredictions: 15, correctPredictions: 10 },
      ];
      const svc = new SeasonService(seasonRepo as any, makeUserRepo(users) as any);
