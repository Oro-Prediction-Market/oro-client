...
function Step({ n, done, text }: { n: number; done: boolean; text: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <span style={{ fontSize: 14, lineHeight: 1.4, color: done ? "#111" : "#6b7280" }}>
        {text}
      </span>
    </div>
  );
...
export const TmaWalletPage: FC = () => {
  ...
  const [linkStep, setLinkStep] = useState<"idle" | "loading" | "success" | "error">("idle");
  ...
  const refreshWallet = () => {
    ...
    getMyTransactions()
      .then((txList) => {
        ...
        const latestReferralBonus = txList.find((t) => t.type === "referral_bonus");
        if (latestReferralBonus) {
          setReferralDepositNudge({
            friendName:
              latestReferralBonus.note?.replace("Referral bonus: ", "").split(" ")[0] ??
              "Your friend",
  ...
  const handlePaymentInitiate = async () => {
    ...
    if (!Number.isFinite(amount) || amount < minAmt) {
      setPayError(`Minimum ${paymentModal === "deposit" ? "top-up" : "cash out"} is Nu ${minAmt}.`);
      return;
    }
    setPayError("");
    setPayProcessing(true);
    if (paymentModal === "deposit") {
      depositPrevBalance.current = Number(freshUser?.creditsBalance ?? user?.creditsBalance ?? 0);
    }
    try {
      let res;
      if (paymentModal === "deposit") {
    ...
    <Page>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 0 100px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={balanceCard}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.6)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4, display: "flex", alignItems: "center", gap: 8 }}>
            Available Balance
            <button
              onClick={() => setBalanceHidden((h) => !h)}
              style={{ background: "none", border: "none", padding: 0, cursor: "pointer", color: "rgba(255,255,255,0.55)", display: "flex", alignItems: "center" }}
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, opacity: balanceLoading ? 0.5 : 1, transition: "opacity 0.3s", marginBottom: 20 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.8)" }}>BTN</span>
            <span style={{
              fontSize: 36,
              fontWeight: 900,
              color: "#fff",
              ...
              textShadow: balanceFlash ? "0 0 24px rgba(34,197,94,0.9)" : "none",
              animation: balanceFlash ? "balanceWin 1.4s ease-out" : "none",
            }}>
              <AnimatedCounter value={Number(freshUser?.creditsBalance ?? user?.creditsBalance ?? 0)} />
            </span>
          </div>
          ...
          <div style={{ display: "flex", gap: 0, borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 14 }}>
            {[
              { label: "Total Won", value: `+${totalWon.toLocaleString()}`, color: "#6ee7b7" },
              { label: "Deposited", value: `+${totalDeposited.toLocaleString()}`, color: "rgba(255,255,255,0.7)" },
              { label: "This Week", value: `${weeklyProfit >= 0 ? "+" : ""}${weeklyProfit.toLocaleString()}`, color: weeklyProfit >= 0 ? "#6ee7b7" : "#fca5a5" },
            ].map((s, i) => (
              <div key={i} style={{ flex: 1, borderLeft: i > 0 ? "1px solid rgba(255,255,255,0.12)" : "none", paddingLeft: i > 0 ? 14 : 0 }}>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", fontWeight: 600, marginBottom: 3, textTransform: "uppercase", letterSpacing: "0.04em" }}>{s.label}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "0 16px" }}>
          <button style={actionBtnPrimary} onClick={() => openPaymentModal("deposit")}>
            <Plus size={16} /> Top Up
          </button>
          <button style={actionBtnSecondary} onClick={() => openPaymentModal("withdraw")}>
        ...
        {isFirstDeposit && (
          <div
            style={{ margin: "0 16px", borderRadius: 14, padding: "14px 16px", background: "linear-gradient(135deg, #f59e0b 0%, #fbbf24 40%, #f59e0b 70%, #d97706 100%)", backgroundSize: "200% auto", animation: "bonusBannerShimmer 3s linear infinite, nudgePulse 2.5s ease-in-out infinite", boxShadow: "0 4px 16px rgba(245,158,11,0.35)", cursor: "pointer" }}
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ background: "#1c1917", color: "#fbbf24", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 800, whiteSpace: "nowrap" }}>Claim →</div>
            </div>
          </div>
        )}
          ...
          <div style={{ margin: "0 16px", borderRadius: 14, padding: "12px 14px", background: "linear-gradient(135deg, rgba(34,197,94,0.12), rgba(16,185,129,0.06))", border: "1px solid rgba(34,197,94,0.3)", animation: "nudgePulse 3s ease-in-out infinite", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--text-subtle)", marginTop: 2 }}>
                You earned <strong style={{ color: "#4ade80" }}>Nu {referralDepositNudge.bonusEarned.toLocaleString()}</strong> instantly!
              </div>
            </div>
            <button onClick={() => setReferralDepositNudge(null)} style={{ background: "none", border: "none", color: "var(--text-subtle)", cursor: "pointer", padding: 4, display: "flex", alignItems: "center" }}>
              <X size={14} />
            </button>
          </div>
        )}
          ...
          <div style={{ margin: "0 16px", borderRadius: 14, padding: "12px 14px", background: "linear-gradient(135deg, rgba(59,130,246,0.1), rgba(99,102,241,0.06))", border: "1px solid rgba(99,102,241,0.25)", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 5 }}>
                {Array.from({ length: Math.min(depositStreakDays, 7) }).map((_, i) => (
                  <div key={i} style={{ width: 22, height: 8, borderRadius: 4, background: i < depositStreakDays ? "linear-gradient(90deg, #6366f1, #818cf8)" : "var(--glass-border)", transition: "background 0.3s" }} />
                ))}
                {depositStreakDays < 7 && <div style={{ width: 22, height: 8, borderRadius: 4, background: "var(--glass-border)", opacity: 0.4 }} />}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 18, fontWeight: 900, color: "#818cf8" }}>{depositStreakDays}</div>
              <div style={{ fontSize: 10, color: "var(--text-subtle)" }}>day{depositStreakDays !== 1 ? "s" : ""}</div>
            </div>
            <div style={{ fontSize: 10, color: "#6366f1", fontWeight: 700, marginLeft: 2 }}>keep it going →</div>
          </div>
        )}
        ...
        <div style={card}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "var(--text-main)" }}>
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
              {hasDKBank ? <CheckCircle2 size={16} color="#059669" /> : <Link2 size={16} color="#2775d0" />}
              {hasDKBank ? (
          ...
          {hasDKBank ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <p style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: 0, fontSize: 14 }}>
                ...
                <span style={{ display: "flex", alignItems: "center", gap: 5, color: "var(--text-main)", fontWeight: 600, fontSize: 13 }}>
                  {user?.isDkPhoneLinked ? (
                    ...
                    <span style={{ color: "#d97706", display: "flex", alignItems: "center", gap: 5 }}>
                      <AlertCircle size={13} color="#d97706" /> No phone on DK Bank record
                    </span>
                  )}
                </span>
              </p>
            ...
            <>
              <p style={{ margin: 0, fontSize: 14, color: "var(--text-muted)", lineHeight: 1.6 }}>
                Enter your 11-digit Bhutanese National ID (CID) to link your DK Bank account.
              </p>
              <input
                ...
                maxLength={11}
                value={cid}
                onChange={(e) => { setCid(e.target.value.replace(/\D/g, "")); setLinkStep("idle"); setLinkError(""); }}
              />
              {linkStep === "error" && (
                <p style={{ margin: 0, fontSize: 13, color: "#dc2626", display: "flex", alignItems: "center", gap: 5 }}>
                  <XCircle size={14} color="#dc2626" />{linkError}
                </p>
              )}
              {linkStep === "success" && (
                <p style={{ margin: 0, fontSize: 13, color: "#059669", fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <button
                style={{ width: "100%", padding: "14px", fontSize: 15, fontWeight: 700, background: "linear-gradient(135deg, #00499cff, #1a5bb5)", color: "#fff", border: "none", borderRadius: 12, cursor: "pointer", transition: "opacity 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, opacity: linkStep === "loading" || cid.length !== 11 ? 0.6 : 1 }}
                disabled={linkStep === "loading" || cid.length !== 11}
                onClick={handleLink}
              >
                {linkStep === "loading" ? (
                  <><Loader2 size={15} style={{ animation: "spin 0.8s linear infinite" }} /> Linking…</>
                ) : (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <Link2 size={15} /> Link
                    <span style={{ background: "rgba(255,255,255,0.15)", borderRadius: 4, padding: "1px 5px", display: "inline-flex", alignItems: "center" }}>
                      <img src={dkBankLogo} alt="DK Bank" style={{ height: 13, width: "auto" }} />
                    </span>
                    Account
                  </span>
                )}
        ...
        {!hasPhoneVerified && (
          <div style={card}>
            ...
            <p style={{ margin: 0, fontSize: 14, color: "var(--text-muted)", lineHeight: 1.6 }}>
              After linking your DK Bank CID above, go to the Oro bot and send <strong>/verify</strong> to verify your phone number.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <Step n={4} done={hasPhoneVerified} text="Phone verified — payments unlocked!" />
            </div>
          </div>
        )}
        ...
        <div style={{ padding: "0 16px", marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div>
              <h2 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 700, color: "var(--text-main)" }}>Transaction History</h2>
              <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginTop: 3 }}>
                {txLoading ? "Updating…" : `${txs.length} transaction${txs.length !== 1 ? "s" : ""}`}
              </div>
            </div>
          ...
          {txError && !txLoading && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "40px 20px", color: "var(--text-muted)" }}>
          ...
          {!txLoading && !txError && (
            <>
              {(() => {
                ...
                return (
                  <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", marginBottom: 8, borderRadius: 12, background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.18)" }}>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 800, color: "#22c55e" }}>+{totalEarned.toLocaleString()}</div>
              ...
              <div style={{ background: "var(--bg-card)", borderRadius: 16, overflow: "hidden", boxShadow: "var(--shadow-sm)", border: "1px solid var(--glass-border)" }}>
                {txs.length === 0 ? (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "40px 20px", color: "var(--text-muted)" }}>
                  ...
                  txs.slice(0, showAllTxs ? undefined : 5).map((tx) => (
              ...
              {txs.length > 5 && (
                <button
                  ...
                  style={{ width: "100%", padding: "12px", marginTop: 12, background: "var(--bg-card)", border: "1px solid var(--glass-border)", borderRadius: 12, color: "var(--text-main)", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, boxShadow: "var(--shadow-sm)" }}
                >
                  <Clock size={14} />
        ...
        >
          <div style={{ background: "var(--bg-card)", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 -4px 40px rgba(0,0,0,0.25)", paddingBottom: 70 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 20px 12px", borderBottom: "1px solid var(--glass-border)" }}>
              ...
              <button style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", padding: 4, display: "flex", alignItems: "center" }} onClick={closePaymentModal}>
                <X size={18} />
              </button>
            </div>
            ...
            {payStep === "amount" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "20px" }}>
                {paymentModal === "deposit" && !user?.dkCid && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 14px", borderRadius: 10, background: "rgba(217,119,6,0.1)", border: "1px solid rgba(217,119,6,0.3)", fontSize: 13, color: "#d97706" }}>
                ...
                <p style={{ margin: 0, fontSize: 13, color: "var(--text-muted)", fontWeight: 500 }}>
                  {paymentModal === "deposit" ? "Top-up amount (BTN)" : "Cash out amount (BTN)"}
                </p>
                <input
                  style={{ width: "100%", padding: "14px 16px", fontSize: 18, borderRadius: 12, border: "2px solid var(--glass-border)", background: "var(--bg-main)", color: "var(--text-main)", outline: "none", boxSizing: "border-box", fontWeight: 700 }}
                  type="number"
                  inputMode="numeric"
                  min={paymentModal === "deposit" ? MIN_DEPOSIT : MIN_WITHDRAW}
                  placeholder="Enter amount"
                  value={payAmountStr}
                  onChange={(e) => { setPayAmountStr(e.target.value); setPayError(""); }}
                />
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {(paymentModal === "deposit" ? QUICK_DEPOSIT_AMOUNTS : QUICK_WITHDRAW_AMOUNTS).map((amt) => (
                    <button
                      key={amt}
                      style={{ flex: 1, minWidth: 60, padding: "8px 4px", borderRadius: 8, border: payAmountStr === String(amt) ? "1.5px solid #2775d0" : "1.5px solid var(--glass-border)", background: payAmountStr === String(amt) ? "rgba(39,117,208,0.12)" : "var(--bg-main)", color: payAmountStr === String(amt) ? "#2775d0" : "var(--text-muted)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
                      onClick={() => { setPayAmountStr(String(amt)); setPayError(""); }}
                    >
                ...
                {paymentModal === "deposit" && user?.dkCid && (
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderTop: "1px solid var(--glass-border)" }}>
                    <span style={{ color: "var(--text-muted)", fontSize: 13 }}>DK Account</span>
                    <span style={{ fontWeight: 600, fontSize: 13 }}>{user.dkAccountName || user.dkCid}</span>
                  </div>
                )}
                {paymentModal === "withdraw" && (
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderTop: "1px solid var(--glass-border)" }}>
                    <span style={{ color: "var(--text-muted)", fontSize: 13 }}>Available balance</span>
                    <span style={{ fontWeight: 600, fontSize: 13 }}>Nu {Number(freshUser?.creditsBalance ?? user?.creditsBalance ?? 0).toLocaleString()}</span>
                  </div>
                )}
                {payError && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 14px", borderRadius: 10, background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.25)", fontSize: 13, color: "#dc2626" }}>
                    <XCircle size={14} color="#dc2626" /><span>{payError}</span>
                  </div>
                )}
                <button
                  style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #2775d0, #1a5bb5)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: payProcessing || (paymentModal === "deposit" ? !user?.dkCid : !user?.dkCid) ? 0.6 : 1 }}
                  disabled={payProcessing}
                  onClick={handlePaymentInitiate}
                >
                  {payProcessing ? (
                    <><Loader2 size={16} style={{ animation: "spin 0.8s linear infinite" }} /> Sending OTP…</>
                  ) : (
                    <><Send size={16} /> {paymentModal === "deposit" ? "Deposit & Send OTP" : "Withdraw & Send OTP"}</>
                  )}
                </button>
                <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", textAlign: "center" }}>An OTP will be sent to your Telegram bot to confirm this transaction.</p>
              </div>
            )}
            ...
            {payStep === "otp" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "20px", alignItems: "center" }}>
                ...
                <p style={{ margin: 0, fontSize: 13, color: "var(--text-muted)", textAlign: "center", lineHeight: 1.5 }}>
                  We sent a code to your <strong style={{ color: "var(--text-main)" }}>Telegram bot</strong> to confirm <strong style={{ color: "#2775d0" }}>Nu {parseFloat(payAmountStr).toLocaleString()}</strong>
                  {paymentModal === "deposit" && isFirstDeposit && <span style={{ color: "#f59e0b", fontWeight: 700 }}> (+10% bonus applied!)</span>}
                </p>
                <div style={{ display: "flex", gap: 8, justifyContent: "center", margin: "4px 0", width: "100%" }}>
                  {Array.from({ length: 6 }).map((_, i) => {
                    const digit = payOtp[i];
                    const isFilled = !!digit;
                    const isActive = payOtp.length === i;
                    return (
                      <div key={i} style={{ width: 44, height: 54, borderRadius: 12, border: isFilled ? "2px solid #2775d0" : isActive ? "2px solid rgba(39,117,208,0.5)" : "2px solid var(--glass-border)", background: isFilled ? "rgba(39,117,208,0.1)" : "var(--bg-main)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 800, color: "#2775d0", transition: "all 0.15s", animation: isFilled ? "otpDigitPop 0.2s ease" : "none", boxShadow: isActive ? "0 0 0 3px rgba(39,117,208,0.15)" : "none" }}>
                        {digit ?? (isActive ? <span style={{ width: 2, height: 22, background: "#2775d0", borderRadius: 2, animation: "nudgePulse 0.8s ease-in-out infinite" }} /> : "")}
                      </div>
                    );
                  })}
                </div>
                <input style={{ position: "absolute", opacity: 0, pointerEvents: "none", width: 1, height: 1 }} type="text" inputMode="numeric" maxLength={8} value={payOtp} onChange={(e) => { setPayOtp(e.target.value.replace(/\D/g, "")); setPayError(""); }} autoFocus id="otp-hidden-input" />
                <button onClick={() => document.getElementById("otp-hidden-input")?.focus()} style={{ width: "100%", padding: "10px", borderRadius: 10, border: "1.5px dashed var(--glass-border)", background: "transparent", color: "var(--text-subtle)", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                  <Smartphone size={14} /> Tap here to enter OTP
                </button>
                {payError && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 14px", borderRadius: 10, background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.25)", fontSize: 13, color: "#dc2626", width: "100%", boxSizing: "border-box" }}>
                    <XCircle size={14} color="#dc2626" /><span>{payError}</span>
                  </div>
                )}
                <button style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #2775d0, #1a5bb5)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: payProcessing || payOtp.length < 4 ? 0.6 : 1 }} disabled={payProcessing || payOtp.length < 4} onClick={handlePaymentConfirm}>
                  {payProcessing ? <><Loader2 size={16} style={{ animation: "spin 0.8s linear infinite" }} /> Confirming…</> : <><CheckCircle2 size={16} /> Confirm</>}
                </button>
                <button style={{ width: "100%", padding: "12px", borderRadius: 12, border: "1px solid var(--glass-border)", background: "transparent", color: "var(--text-muted)", fontSize: 14, cursor: "pointer" }} onClick={() => { setPayStep("amount"); setPayOtp(""); setPayError(""); }}>
                  ← Change amount
                </button>
              </div>
            )}
            ...
            {/* Step: Success */}
            {payStep === "success" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "20px", alignItems: "center", textAlign: "center", position: "relative", overflow: "hidden" }}>
                ...
                <button style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #2775d0, #1a5bb5)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" }} onClick={closePaymentModal}>Done</button>
              </div>
            )}

            {/* Step: Failed */}
            {payStep === "failed" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "20px", alignItems: "center", textAlign: "center" }}>
                <XCircle size={56} color="#dc2626" />
                <p style={{ fontWeight: 700, fontSize: 18, margin: "0 0 8px" }}>Transaction Failed</p>
                <p style={{ color: "var(--text-muted)", fontSize: 14, margin: "0 0 24px", lineHeight: 1.6 }}>{payError || "The transaction could not be completed. Please try again."}</p>
                <button style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #2775d0, #1a5bb5)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" }} onClick={() => { setPayStep("amount"); setPayError(""); }}>Try Again</button>
                <button style={{ width: "100%", padding: "12px", borderRadius: 12, border: "1px solid var(--glass-border)", background: "transparent", color: "var(--text-muted)", fontSize: 14, cursor: "pointer" }} onClick={closePaymentModal}>Cancel</button>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ── Share Win Modal ─────────────────────────────────── */}
      {shareWinTx && (
        <div onClick={() => setShareWinTx(null)} style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px 16px" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 560, position: "relative" }}>
            <button onClick={() => setShareWinTx(null)} style={{ position: "absolute", top: -36, right: 0, background: "transparent", border: "none", color: "rgba(255,255,255,0.7)", cursor: "pointer", padding: 6 }}>
              <X size={22} />
            </button>
            <BetShareCard
