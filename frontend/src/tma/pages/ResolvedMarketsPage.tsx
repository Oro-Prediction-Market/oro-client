            ...
            >
              ...
              {m.evidence.submittedAt && (
                <div style={{ fontSize: "0.68rem", color: "var(--text-subtle)" }}>
                  Submitted {new Date(m.evidence.submittedAt).toLocaleDateString("en-BT", {
        ...
        <div style={{ padding: "48px 16px 24px", display: "flex", flexDirection: "column", gap: 16, position: "relative" }}>
          ...
          {markets.map((m) => (
            <Link key={m.id} to={`/market/${m.id}`} style={{ textDecoration: "none" }}>
              <div style={{ background: "var(--bg-card)", border: `1px solid ${m.objectionCount > 0 ? (m.outcomeChanged ? "rgba(239,68,68,0.3)" : "rgba(245,158,11,0.3)") : "var(--glass-border)"}`, borderRadius: "var(--radius-lg)", padding: "18px", boxShadow: "var(--shadow-premium)", display: "flex", flexDirection: "column", gap: 10 }}>
